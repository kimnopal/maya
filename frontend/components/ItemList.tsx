...
const ItemList = ({ item }: { item: postType }) => {
  return (
    <Link
      href={`/post/`}
      className="flex flex-row justify-center items-stretch gap-2.5 px-2 py-1.5 bg-white rounded-2xl shadow-lg hover:shadow-2xl"
    >
      <Image
        width={64}
        height={64}
      ...
      <div className="flex flex-col justify-between items-start text-xs">
        <div className="flex flex-row flex-1 self-stretch justify-between">
          <h5 className="h-4 font-bold flex-1 overflow-hidden">{item.title}</h5>
